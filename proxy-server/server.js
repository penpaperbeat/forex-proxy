...
// 1-month chunks + next_url pagination to reliably collect all 7 pairs.
// Checkpointed: complete pairs are skipped on redeploy, incomplete pairs retry.
async function runMassiveBackfill() {
  ...
  // Clear incomplete pairs so they always retry
  ...
  // 1-month chunks — keeps well under 50k candle limit and avoids queryCount truncation
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    for (const chunk of chunks) {
      ...
      try {
        // Paginate through next_url until all candles for this chunk are collected
        ...
        while (nextUrl && pages < 10) {
          ...
          for (const bar of results) {
            ...
            // Filter outliers: skip candles with >20% price jump from previous close
    ...
    // Merge with existing candle store — all sources coexist, nothing overwritten
    if (!candleStore.pairs[pair]) candleStore.pairs[pair] = [];
    const mapByDt = {};
...
app.listen(PORT, () => {
  ...
  // Massive backfill — runs once on startup, checkpointed per pair
  setImmediate(() => runMassiveBackfill());

  // Intelligence profile — 2 min after startup, then every 7 days
