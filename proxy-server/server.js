...
// --- Live collection (Twelve Data — unchanged) ---
...
// --- Calendar (Finnhub / ForexFactory fallback — unchanged) ---
...
// --- Massive.com backfill (replaces Dukascopy) ---
// Uses the /v2/aggs/ticker/{ticker}/range/1/hour/{from}/{to} endpoint.
// Free plan: 2 years of history, unlimited API calls, end-of-day data.
// One call per 6-month chunk per pair to stay well under the 50k candle limit.
async function runMassiveBackfill() {
  ...
  // Build 6-month chunks to stay under 50k candle limit per call
  // 6 months * 30 days * 24 hours = ~4,320 candles — well within limit
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    for (const chunk of chunks) {
      ...
      try {
        // Massive aggregate endpoint: 1-hour bars
        const url = `https://api.massive.com/v2/aggs/ticker/${ticker}/range/1/hour/${chunk.from}/${chunk.to}`;
        ...
        for (const bar of results) {
          ...
          // Filter outliers: skip candles with >20% price jump
        ...
        // Polite delay between chunks — Massive free tier has no rate limit but we avoid hammering
    ...
    // Merge with existing candle store
    if (!candleStore.pairs[pair]) candleStore.pairs[pair] = [];
    const mapByDt = {};
...
app.listen(PORT, () => {
  ...
  // Live collection + SMC every 15 minutes
  ...
  // Massive backfill — runs once on startup, checkpointed
  setImmediate(() => runMassiveBackfill());

  // Intelligence profile — first run 2 min after startup, then every 7 days
  setTimeout(() => calculateIntelligenceProfile(), 2 * 60 * 1000);
  setInterval(() => calculateIntelligenceProfile(), 7 * 24 * 60 * 60 * 1000);

  // Ensemble training from canister
  setTimeout(async () => {
