...
async function runDukascopyBackfill() {
  ...
  try {
    console.log('[backfill] Loading dukascopy-node...');
    ({ getHistoricalRates } = require('dukascopy-node'));
    console.log('[backfill] dukascopy-node loaded successfully.');
  } catch (err) {
    backfillStatus = 'error';
    console.error('[backfill] dukascopy-node not available:', err.message);
    return;
  }
  console.log('[backfill] Beginning pair loop...');
  let progress = {};
  ...
  for (const pair of FOREX_PAIRS) {
    if (progress[pair] === 'complete') { backfillPairsComplete++; console.log(`[backfill] ${pair}: already complete, skipping.`); continue; }
    ...
    console.log(`[backfill] Starting ${pair} (instrument: ${instrument})...`);
    for (let year = startYear; year <= now.getUTCFullYear(); year++) {
      ...
      } catch (err) { yearsFailed++; console.error(`[backfill] ${pair} / ${year} failed:`, err.message); if (yearsFailed >= 3) { console.error(`[backfill] ${pair}: too many failures, skipping pair.`); break; } }
      await new Promise(r => setTimeout(r, 500));
    }
    if (pairCandles.length === 0) { progress[pair] = 'incomplete'; console.warn(`[backfill] ${pair}: 0 candles collected.`); try { fs.writeFileSync(BACKFILL_PROGRESS_PATH, JSON.stringify(progress), 'utf8'); } catch (_) {} continue; }
    if (!candleStore.pairs[pair]) candleStore.pairs[pair] = [];
    const mapByDt = {};
    ...
    console.log(`[backfill] ${pair}: complete. ${pairCandles.length} candles stored.`);
  ...
  console.log(`[backfill] All pairs done. Status: ${backfillStatus}. Total candles: ${backfillTotalCandles}`);
}
