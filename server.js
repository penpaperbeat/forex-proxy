...
function resolveOldSignals() {
  ...
  for (const sig of paperSignalsBuffer) {
    ...
    if (age >= 24 * 60 * 60 * 1000) {
      ...
      resolved++;
      // Phase 1: use soft outcome label instead of binary win/loss
      const pairIntel = intelligenceProfile && intelligenceProfile[sig.pair];
      const softLabel = ensembleScorer.computeSoftLabel(sig, sig.currentPriceAtResolution || sig.entryPrice);
...
async function fetchCalendar() {
  ...
  const CALENDAR_STALE_MS = 12 * 60 * 60 * 1000;
...
async function runDukascopyBackfill() {
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    for (let year = startYear; year <= now.getUTCFullYear(); year++) {
      ...
      const yearEnd   = year === now.getUTCFullYear() ? now.toISOString() : new Date(Date.UTC(year + 1, 0, 1)).toISOString();
      ...
      if (!yearSuccess) {
        ...
        if (yearsFailed >= 3) { console.error(`[backfill] ${pair}: too many year failures, skipping pair.`); break; }
        continue;
      }
      try {
        ...
        for (const row of raw) {
          ...
          if (prev !== null) {
            ...
            if (change > 0.20) { console.warn(`[backfill] ${pair} outlier rejected: close=${close}, prev=${prev}`); continue; }
          }
    ...
    historyCache.data.pairs[pair] = merged.slice(-5000);
    try { atomicWriteJSON(CANDLE_STORE_PATH, candleStore); } catch (err) { console.error('[backfill] Failed to persist candle store:', err.message); }
...
async function calculateIntelligenceProfile() {
  ...
  for (const pair of FOREX_PAIRS) {
    ...
    for (const sess of sessions) {
      ...
      const confidence = sampleSize >= 500 ? 'high' : sampleSize >= 200 ? 'medium' : sampleSize >= 100 ? 'low' : 'insufficient';
    ...
    const seasonalBias = {};
    const avgMove = Object.values(monthBuckets).filter(b => b.totalWeight > 0).reduce((s, b) => s + b.moveWeight / b.totalWeight, 0) / 12 || 1;
    for (let m = 1; m <= 12; m++) {
      const b = monthBuckets[m];
      seasonalBias[String(m)] = b.totalWeight > 0 ? Math.round((b.moveWeight / b.totalWeight / avgMove) * 100) / 100 : 1.00;
    }
    profile[pair] = { adaptiveRSI, atrPercentiles, sessionMatrix: finalSessionMatrix, seasonalBias, volatilityRegime, profileVersion: 1, lastCalculated: new Date().toISOString(), status: 'active' };
    calculated++;
...
async function runSMCEvaluation() {
  ...
  for (const pair of FOREX_PAIRS) {
    try {
      ...
      // ---- HTF Confluence Gate (Phase 1) ----
      const htfBias     = smcEngine.getHTFBias(candles);
      const htfConflict = smcEngine.htfConflictCheck(candidateDirection, htfBias);
      ...
      // classification must be let so HTF gate can demote it
      let classification = smcEngine.classifySignal(trinity, indicatorPassed, killzone);

      // Demote LIVE/STANDARD to PAPER_INDICATOR_FAILED if HTF conflicts
      if (htfConflict) {
        if (classification === 'LIVE' || classification === 'STANDARD') {
          console.log(`[SMC] HTF conflict: ${pair} ${candidateDirection} demoted to PAPER (h4=${htfBias.h4Bias}, daily=${htfBias.dailyBias})`);
          classification = 'PAPER_INDICATOR_FAILED';
        } else {
          console.log(`[SMC] HTF conflict (already paper): ${pair} ${candidateDirection} (h4=${htfBias.h4Bias}, daily=${htfBias.dailyBias})`);
        }
      }
      ...
      const signalBase = {
        ...
        // Phase 1: HTF bias fields stored on every signal
        htfH4Bias:              htfBias.h4Bias,
        htfDailyBias:           htfBias.dailyBias,
        htfConflict
      };
      ...
      if (classification === 'LIVE') {
        ...
        if (demoteToPaper) {
          const paperSignal = { ...signalBase, ensembleScore, isPaper: true, paperReason: 'ENSEMBLE_LOW_SCORE' };
          if (paperSignalsBuffer.length >= PAPER_SIGNALS_MAX) { const dropped = paperSignalsBuffer.shift(); console.log('[paper] Buffer full (500), dropping oldest signal:', dropped && dropped.id); }
          paperSignalsBuffer.push(paperSignal);
          paperSignalsPushCounter++;
          if (paperSignalsPushCounter % 10 === 0) { pushPaperSignalsToCanister(paperSignalsBuffer).catch(() => {}); }
          console.log(`[SMC] LIVE demoted to PAPER (ensembleScore=${ensembleScore}): ${pair} ${candidateDirection}`);
        } else {
          lastHolyTrinityAt[pair] = now;
          const liveSignal = { ...signalBase, ensembleScore, highConviction, isPaper: false };
        ...
        const demoteToPaper = ensembleScore !== null && ensembleScore < 40;
        if (demoteToPaper) {
          const paperSignal = { ...signalBase, confidence: Math.min(70, adjustedConfidence), ensembleScore, isPaper: true, paperReason: 'ENSEMBLE_LOW_SCORE' };
          if (paperSignalsBuffer.length >= PAPER_SIGNALS_MAX) { const dropped = paperSignalsBuffer.shift(); console.log('[paper] Buffer full (500), dropping oldest signal:', dropped && dropped.id); }
          paperSignalsBuffer.push(paperSignal);
          paperSignalsPushCounter++;
          if (paperSignalsPushCounter % 10 === 0) { pushPaperSignalsToCanister(paperSignalsBuffer).catch(() => {}); }
          ...
          const standardSignal = { ...signalBase, confidence: Math.min(70, adjustedConfidence), ensembleScore, highConviction, isPaper: false };
      ...
      if (holyTrinityAge > 24 && classification !== 'LIVE' && adjustedConfidence >= 50) {
        ...
        try {
          const fallbackBase = { ...signalBase, confidence: Math.min(70, adjustedConfidence), isPaper: false, signalTypeKey: 0, orderBlockPresent: false, fvgPresent: false, liquiditySweepPresent: false, obZone: null, fvgZone: null, isFallback: true };
          const fbSweepAge = (sweepResult && sweepResult.sweepCandleIndex != null) ? candles.length - 1 - sweepResult.sweepCandleIndex : undefined;
        ...
        const fallbackSignal = { ...signalBase, confidence: Math.min(70, adjustedConfidence), ensembleScore: fallbackEnsembleScore, highConviction: fallbackHighConviction, isPaper: false, signalTypeKey: 0, orderBlockPresent: false, fvgPresent: false, liquiditySweepPresent: false, obZone: null, fvgZone: null, isFallback: true };
...
async function pushSignalToCanister(signal) {
  ...
  try {
    const direction = (signal.direction || '').toUpperCase() === 'SELL' ? { 'Sell': null } : { 'Buy': null };
    ...
    if (!_candles || _candles.length < 15) { console.warn(`[canister] Skipping signal push for ${signal.pair}: insufficient candles for ATR`); return; }
    const _atrs = calculateATR(_candles, 14);
    const _atr = _atrs.slice().reverse().find(v => v != null && !isNaN(v)) || 0;
    if (_atr === 0) { console.warn(`[canister] Skipping signal push for ${signal.pair}: ATR is zero`); return; }
    ...
    const input = {
      ...
      dxyBias: signal.dxyBias || 'UNKNOWN', dxyStale: signal.dxyStale === true,
      fvgPresent: signal.fvgPresent === true, fvgZoneHigh: Number(fvgZone.high || 0), fvgZoneLow: Number(fvgZone.low || 0),
      orderBlockPresent: signal.orderBlockPresent === true, obZoneHigh: Number(obZone.high || 0), obZoneLow: Number(obZone.low || 0),
    ...
    if ('ok' in result) { console.log('[canister] Signal pushed:', signal.pair, signal.direction, '— total:', result.ok); }
    else { console.error('[canister] Signal push rejected:', result.err); }
  } catch (e) { console.error('[canister] Signal push failed:', e.message); }
}
  ...
  for (const pair of Object.keys(candleSnapshot.pairs || {})) {
    ...
    try {
      const candleRecords = candles.slice(-200).map(c => {
        const dt = c.datetime ? c.datetime : new Date(c.timestamp || 0).toISOString();
        const vol = (c.volume != null && c.volume !== 0) ? [Number(c.volume)] : [];
        return { datetime: dt, open: Number(c.open || 0), high: Number(c.high || 0), low: Number(c.low || 0), close: Number(c.close || 0), volume: vol };
      });
      const ok = await canisterActor.pushCandles(pair, candleRecords);
      if (ok === true) { totalPushed += candleRecords.length; }
      else { console.error('[canister] Candles push rejected for pair:', pair); }
    } catch (e) { console.error('[canister] Candles push failed:', pair, e.message); }
  }
...
async function pushResolvedOutcomeToCanister(outcome) {
  ...
  try {
    const result = await canisterActor.pushResolvedOutcome({
      pair: outcome.pair || '', direction: outcome.direction || '',
      entryPrice: outcome.entryPrice || 0, stopLoss: outcome.stopLoss || 0, takeProfit: outcome.takeProfit || 0,
      result: outcome.result || 'EXPIRED', confidence: outcome.confidence || 0,
      timestamp: typeof outcome.timestamp === 'bigint' ? outcome.timestamp : BigInt(Math.round(outcome.timestamp || Date.now() * 1_000_000)),
    });
    if (result && result.err) { console.warn('[canister] pushResolvedOutcome rejected:', result.err); }
    else { console.log('[canister] Pushed resolved outcome:', outcome.pair, outcome.result); }
  } catch (err) { console.warn('[canister] pushResolvedOutcome failed (non-blocking):', err.message || err); }
}

async function restoreResolvedOutcomesFromCanister() {
  if (!canisterActor) return;
  if (resolvedSignalsBuffer.length >= 40) return;
  try {
    const result = await canisterActor.getResolvedOutcomes();
    if (!result || result.err) { console.warn('[canister] getResolvedOutcomes returned error:', result?.err); return; }
    ...
    for (const co of canisterOutcomes) {
      ...
      const alreadyHave = resolvedSignalsBuffer.some(r => r._canisterKey === key);
      if (!alreadyHave) {
        resolvedSignalsBuffer.push({ features: [0], outcome: co.result === 'TP_HIT' ? 1 : 0, _canisterKey: key });
        merged++;
      }
    }
    if (merged > 0) { console.log(`[canister] Restored ${merged} resolved outcomes from canister`); saveResolvedSignals(); }
  } catch (err) { console.warn('[canister] restoreResolvedOutcomesFromCanister failed (non-blocking):', err.message || err); }
}

async function pushModelStateToCanister(modelState) {
  if (!canisterActor) return;
  try {
    const payload = { weights: modelState.weights || [], bias: modelState.bias || 0, means: modelState.means || [], stds: modelState.stds || [], oosAccuracy: modelState.oosAccuracy || 0, trainedAt: BigInt(modelState.trainedAt || Date.now()) };
    const result = await canisterActor.pushModelState(payload);
    if ('ok' in result) { console.log('[canister] Model state pushed'); }
    else { console.warn('[canister] pushModelState err:', result.err); }
  } catch (e) { console.warn('[canister] pushModelState failed:', e.message); }
}
...
async function restoreModelStateFromCanister() {
  ...
  try {
    ...
    if ('ok' in result && result.ok.length > 0) {
      ...
      return { weights: Array.from(state.weights), bias: state.bias, means: Array.from(state.means), stds: Array.from(state.stds), oosAccuracy: state.oosAccuracy, trainedAt: Number(state.trainedAt) };
    }
  } catch (e) { console.warn('[canister] restoreModelState failed:', e.message); }
  return null;
}
...
async function pushIntelligenceProfilesToCanister(profiles) {
  ...
  try {
    const payload = Object.entries(profiles).map(([pair, p]) => ({ pair, volatilityRegime: p.volatilityRegime || 'normal', adxClass: p.adxClass || 'ranging', sessionQuality: p.sessionQuality || 0.5, updatedAt: BigInt(p.updatedAt || Date.now()) }));
    const result = await canisterActor.pushIntelligenceProfiles(payload);
    if ('ok' in result) { console.log('[canister] Intelligence profiles pushed:', payload.length, 'pairs'); }
    else { console.warn('[canister] pushIntelligenceProfiles err:', result.err); }
  } catch (e) { console.warn('[canister] pushIntelligenceProfiles failed:', e.message); }
}
...
async function restoreIntelligenceProfilesFromCanister() {
  ...
  try {
    ...
    if ('ok' in result && result.ok.length > 0) {
      const profiles = {};
      for (const p of result.ok) { profiles[p.pair] = { volatilityRegime: p.volatilityRegime, adxClass: p.adxClass, sessionQuality: p.sessionQuality, updatedAt: Number(p.updatedAt) }; }
      console.log('[canister] Intelligence profiles restored:', Object.keys(profiles).length, 'pairs');
      return profiles;
    }
  } catch (e) { console.warn('[canister] restoreIntelligenceProfiles failed:', e.message); }
  return null;
}
...
async function pushDxyStateToCanister(dxyState) {
  ...
  try {
    const payload = { trend: dxyState.trend || 'neutral', dataPoints: (dxyState.dataPoints || []).map(Number), fetchedAt: BigInt(dxyState.fetchedAt || Date.now()) };
    const result = await canisterActor.pushDxyState(payload);
    if ('ok' in result) { console.log('[canister] DXY state pushed, trend:', dxyState.trend); }
    else { console.warn('[canister] pushDxyState err:', result.err); }
  } catch (e) { console.warn('[canister] pushDxyState failed:', e.message); }
}
...
async function restoreDxyStateFromCanister() {
  ...
  try {
    ...
    if ('ok' in result && result.ok.length > 0) {
      ...
      return { trend: state.trend, dataPoints: Array.from(state.dataPoints).map(Number), fetchedAt: Number(state.fetchedAt) };
    }
  } catch (e) { console.warn('[canister] restoreDxyState failed:', e.message); }
  return null;
}
...
async function pushPaperSignalsToCanister(paperSignals) {
  ...
  try {
    const payload = paperSignals.slice(-200).map(s => ({ pair: s.pair || '', direction: s.direction || 'Buy', entryPrice: s.entryPrice || 0, sl: s.sl || 0, tp: s.tp || 0, confidence: s.confidence || 0, timestamp: BigInt(s.timestamp || Date.now()), smcPatterns: s.smcPatterns || [] }));
    const result = await canisterActor.pushPaperSignals(payload);
    if ('ok' in result) { console.log('[canister] Paper signals pushed:', payload.length); }
    else { console.warn('[canister] pushPaperSignals err:', result.err); }
  } catch (e) { console.warn('[canister] pushPaperSignals failed:', e.message); }
}
...
async function restorePaperSignalsFromCanister() {
  ...
  try {
    ...
    if ('ok' in result && result.ok.length > 0) {
      const signals = result.ok.map(s => ({ pair: s.pair, direction: s.direction, entryPrice: s.entryPrice, sl: s.sl, tp: s.tp, confidence: s.confidence, timestamp: Number(s.timestamp), smcPatterns: Array.from(s.smcPatterns) }));
      console.log('[canister] Paper signals restored from canister:', signals.length);
      return signals;
    }
  } catch (e) { console.warn('[canister] restorePaperSignals failed:', e.message); }
  return null;
}

const publicLimiter = rateLimit({ windowMs: 60 * 1000, max: 60, standardHeaders: true, legacyHeaders: false, message: { error: 'Too many requests, please try again later.' } });
...
app.get('/ensemble-status', requireSecret, (req, res) => { cors(res); res.json(ensembleScorer.getEnsembleStatus()); });
...
app.get('/smc-status', (req, res) => {
  ...
  try {
    ...
    for (const pair of FOREX_PAIRS) {
      ...
      if (sweep.sweepCandleIndex !== null) { lastSweepAge = candles.length - 1 - sweep.sweepCandleIndex; }
      const pdLabel = pdZone.isPremium ? 'PREMIUM' : pdZone.isDiscount ? 'DISCOUNT' : 'NEUTRAL';
      activePairs[pair] = { activeOBCount: obsBuy.length + obsSell.length, activeFVGCount: fvgsBuy.length + fvgsSell.length, lastSweepAge, premiumDiscount: pdLabel };
    }
    res.json({ dxyBias: currentDXYBias, dxyLastFetched: dxyLastFetchedAt || (dxyCache.fetchedAt ? new Date(dxyCache.fetchedAt).toISOString() : null), killzoneActive: killzone.active, killzoneName: killzone.killzoneName, activePairs });
  } catch (err) { console.error('[SMC] /smc-status error:', err.message); res.status(500).json({ error: 'SMC status unavailable' }); }
});
...
app.get('/stored-history', requireSecret, publicLimiter, (req, res) => {
  ...
  if (req.query.limit !== undefined && (isNaN(rawLimit) || rawLimit <= 0)) { return res.status(400).json({ error: 'Limit must be a positive integer' }); }
...
app.get('/calendar', publicLimiter, (req, res) => { cors(res); res.json({ events: calendarCache.data, fetchedAt: calendarCache.fetchedAt ? new Date(calendarCache.fetchedAt).toISOString() : null, count: calendarCache.data.length }); });
...
app.get('/status', publicLimiter, (req, res) => {
  ...
  res.json({ callsToday: dailyCallCount, limit: 800, resetAt: new Date(callCountResetAt).toUTCString(), candlePairsStored: Object.keys(candleStore.pairs).length, candleCountPerPair: storeCounts, calendarEvents: calendarCache.data.length, calendarFetchedAt: calendarCache.fetchedAt ? new Date(calendarCache.fetchedAt).toISOString() : null, backfillStatus, backfillPairsComplete, backfillPairsTotal: FOREX_PAIRS.length, backfillTotalCandles, intelligenceStatus, intelligenceLastCalculated, lastCollectionError: lastCollectionError || null });
});

app.get('/health', publicLimiter, (req, res) => { cors(res); res.json({ status: 'ok', uptime: process.uptime(), ratesCachedAt: ratesCache.fetchedAt ? new Date(ratesCache.fetchedAt).toISOString() : null, historyCachedAt: historyCache.fetchedAt ? new Date(historyCache.fetchedAt).toISOString() : null, calendarCachedAt: calendarCache.fetchedAt ? new Date(calendarCache.fetchedAt).toISOString() : null, intelligenceStatus }); });
...
app.listen(PORT, () => {
  ...
  const runCollectionWithSMC = async () => {
    if (isCollectionRunning) {
      ...
      if (elapsed >= 10 * 60 * 1000) { console.warn('[collection] Safety timeout: force-resetting after', Math.round(elapsed / 1000), 's'); isCollectionRunning = false; }
      else { console.warn('[collection] Cycle already running — skipping this tick.'); return; }
    }
  ...
  smcEngine.fetchDXYData()
    .then(data => {
      ...
      dxyLastFetchedAt = new Date().toISOString();
      dxyCache = { trend: currentDXYBias, dataPoints: data && Array.isArray(data) ? data.map(d => d.close || d.value || 0).slice(-50) : [], fetchedAt: Date.now() };
      pushDxyStateToCanister(dxyCache).catch(() => {});
  ...
  initCanisterActor().then(async () => {
    ...
    // Phase 1: threshold updated to 40 to match ensembleScorer
    if (resolvedSignalsBuffer.length >= 40) {
      ...
      console.log('[Ensemble] Insufficient resolved signals on startup (' + resolvedSignalsBuffer.length + '). Model will activate once 40+ outcomes are available.');
    }
